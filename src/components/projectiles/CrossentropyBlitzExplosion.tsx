...
const ENTROPY_RING_SCALES = [0.45, 0.65, 0.85];
...
export default function CrossentropyBlitzExplosion({
  position,
  chargeTime = 0.25,
  ...
  const geometries = useMemo(() => {
    ...
    const tubeOuter = Math.max(0.06, radius * 0.11);
    const tubeInner = tubeOuter * 0.65;
    const entropyTube = Math.max(0.04, radius * 0.036);
    return {
      ...
      crossPlane: new PlaneGeometry(radius * 1.4, radius * 1.0),
      core: new SphereGeometry(radius * 0.2, 12, 12),
      spark: new SphereGeometry(0.05, 6, 6),
  ...
  useFrame((state) => {
    ...
    blastLight.current?.setPosition(px, py + 0.5, pz);
    blastLight.current?.setIntensity(5 * fade * (1 + t * 0.4));
    ...
    sparkRefs.current.forEach((spark, i) => {
      ...
      const sparkY = Math.sin(t * Math.PI * 2 + i) * 0.25 * fade;
      spark.position.set(
  ...
  return (
    <group ref={groupRef} position={[px, py, pz]}>
      ...
      <mesh
        ref={ringOuterRef}
        rotation={[Math.PI / 2, 0, 0]}
        geometry={geometries.torusOuter}
        material={vfxMaterials.ringOuter}
      />

      <mesh
        ref={ringInnerRef}
        rotation={[Math.PI / 2, 0, 0.35]}
        geometry={geometries.torusInner}
        material={vfxMaterials.ringInner}
      />
