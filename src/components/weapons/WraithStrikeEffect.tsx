...
import { getWraithStrikeColorPalette } from '@/utils/wraithStrikeColorThemes';
import { useMortalStrikeAnimation } from './useMortalStrikeAnimation';

interface WraithStrikeEffectProps {
  position: Vector3;
  direction: Vector3;
  theme?: string;
  onComplete: () => void;
...
export default function WraithStrikeEffect({
  position,
  direction,
  theme,
  onComplete,
}: WraithStrikeEffectProps) {
  const contentRef = useRef<Group>(null);
  const doneRef = useRef(false);

  const palette = useMemo(() => getWraithStrikeColorPalette(theme), [theme]);
