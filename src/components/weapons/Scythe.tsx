...
function ScytheModel({ 
  ...
  const ring4Ref = useRef<Group>(null);
  const ring5Ref = useRef<Group>(null);
  const ring6Ref = useRef<Group>(null);

  // Drive all ring rotations in a single useFrame (replaces invalid useFrame-as-JSX-prop pattern)
  useFrame((state) => {
    const t = state.clock.getElapsedTime();
    if (ring1Ref.current) ring1Ref.current.rotation.x =  t * 2;
    if (ring2Ref.current) ring2Ref.current.rotation.x = -t * 2;
    if (ring3Ref.current) ring3Ref.current.rotation.x = -t * 2;
    if (ring4Ref.current) ring4Ref.current.rotation.x = -t * 2;
