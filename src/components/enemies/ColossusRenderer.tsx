...
import ColossusModel from './ColossusModel';
...
const ATTACK_DURATION = 1260;
const HP_BAR_WIDTH = 4.2;
const LERP_SPEED = 8;
const WALK_STOP_DELAY = 300;

function ColossusRenderer({
  id,
  position,
  rotation,
  health,
  ...
  useEffect(() => {
    ...
    const handleColossusWhiff = (data: { colossusId: string }) => {
      if (data.colossusId !== id) return;
      setMeleeTelegraph((prev) => (prev ? { ...prev, whiffed: true } : prev));
    };
    socket.on('colossus-attack-telegraph', handleTelegraph);
    socket.on('colossus-attack-whiff', handleColossusWhiff);
    return () => {
      socket.off('colossus-attack-telegraph', handleTelegraph);
      socket.off('colossus-attack-whiff', handleColossusWhiff);
    };
  ...
  return (
    <group ref={setGroupRef}>
      ...
      <Billboard position={[0, 5.8, 0]} follow lockX={false} lockY={false} lockZ={false}>
        {health > 0 && !isDying && (
          <>
            <EnemyHpBarPlanes
              fillRef={hpFillRef}
...
export default React.memo(ColossusRenderer);
