...
function ShrineRenderer({
  ...
  const dimmed = (!powered || shrineUsed) && !isDying;
    ...
    <group ref={setGroupRef} rotation={[0, rotation, 0]} visible={!isDying || opacity.current > 0.02}>
      ...
      {dimmed && (
        <mesh position={[0, 2.2, 0]}>
          <cylinderGeometry args={[1.5, 1.7, 4.4, 10]} />
          <meshBasicMaterial color="#0b1220" transparent opacity={0.38} depthWrite={false} />
        </mesh>
      )}
      <Billboard position={[0, SHRINE_HP_BAR_Y, 0]} follow lockX={false} lockY={false} lockZ={false}>
        {health > 0 && !isDying && (
          <>
            <EnemyHpBarPlanes
              fillRef={hpFillRef}
