...
interface AlliedSiegeGolemRendererProps {
  id: string;
  position: Position3;
  rotation: number;
  health: number;
...
function AlliedSiegeGolemRenderer({
  id,
  position,
  rotation,
  health,
  ...
  useEffect(() => {
    ...
    const handleTelegraph = (data: {
      ...
      if (data.siegeGolemId !== id) return;
    ...
    const handleWhiff = (data: { siegeGolemId: string }) => {
      if (data.siegeGolemId !== id) return;
      setMeleeTelegraph((prev) => (prev ? { ...prev, whiffed: true } : prev));
    };
    socket.on('allied-siege-golem-attack-telegraph', handleTelegraph);
    socket.on('allied-siege-golem-attack-whiff', handleWhiff);
    return () => {
      socket.off('allied-siege-golem-attack-telegraph', handleTelegraph);
      socket.off('allied-siege-golem-attack-whiff', handleWhiff);
    };
        ...
        {health > 0 && !isDying && (
          <>
            ...
            <EnemyHealthBarTextLabel
              name={getEnemyDisplayName('allied-siege-golem')}
...
export default React.memo(AlliedSiegeGolemRenderer);
