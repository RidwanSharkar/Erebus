...
import EnemyMeleeAttackRangeRing, { DEATH_KNIGHT_MELEE_ATTACK_RANGE } from './EnemyMeleeAttackRangeRing';
...
interface DeathKnightRendererProps {
  id: string;
  position: Position3;
  rotation: number;
  health: number;
...
function DeathKnightRenderer({
  ...
  useEffect(() => {
    ...
    const handleDeathKnightTelegraph = (data: {
    ...
    const handleDeathKnightWhiff = (data: { deathKnightId?: string }) => {
      if (data.deathKnightId !== id) return;
      setMeleeTelegraph((prev) => (prev ? { ...prev, whiffed: true } : prev));
    };

    const clearHeartstrikeFailsafe = () => {
      if (heartstrikeFailsafeTimer.current) {
        clearTimeout(heartstrikeFailsafeTimer.current);
        heartstrikeFailsafeTimer.current = null;
      }
    };
    ...
    const handleHeartstrikeTelegraph = (data: {
      ...
      setMeleeTelegraph(null);
      isAttackingRef.current = false;
      setHeartstrikeVariant(data.heartstrikeVariant === 2 ? 2 : 1);
      setIsHeartstriking(true);
      isHeartstrikingRef.current = true;
      isWalkingRef.current = false;
      setIsWalking(false);
      clearHeartstrikeFailsafe();
      const duration = data.durationMs ?? HEARTSTRIKE_DURATION_MS;
      heartstrikeFailsafeTimer.current = setTimeout(() => {
    ...
    const handleHeartstrikeEnd = (data: { deathKnightId?: string }) => {
      if (data.deathKnightId !== id) return;
      clearHeartstrikeFailsafe();
      setIsHeartstriking(false);
      isHeartstrikingRef.current = false;
        ...
        {health > 0 && !isDying && (
          <>
            ...
            <EnemyHealthBarTextLabel
              name={getEnemyDisplayName('death-knight')}
...
export default React.memo(DeathKnightRenderer);
