...
import SpiritLounge, { SPIRIT_LOUNGE_HP_BAR_Y } from '@/components/environment/SpiritLounge';
import type { Position3 } from '@/utils/position3';

interface BarracksRendererProps {
  id: string;
  position: Position3;
  rotation: number;
  health: number;
...
function BarracksRenderer({
  ...
  return (
    <group ref={setGroupRef} rotation={[0, rotation, 0]} visible={!isDying || opacity.current > 0.02}>
      ...
      <Billboard position={[0, SPIRIT_LOUNGE_HP_BAR_Y, 0]} follow lockX={false} lockY={false} lockZ={false}>
        {health > 0 && !isDying && (
          <>
            <EnemyHpBarPlanes
              fillRef={hpFillRef}
...
export default React.memo(BarracksRenderer);
