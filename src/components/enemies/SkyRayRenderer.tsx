...
import SkyRayModel from './SkyRayModel';
import EnemyMeleeAttackRangeRing, { SKYRAY_MELEE_ATTACK_RANGE } from './EnemyMeleeAttackRangeRing';
...
const ATTACK_DURATION = 1200; // ms — matches backend SKYRAY_SWING_LOCK_MS
const FADE_DURATION = 1.5;
const LERP_SPEED = 14;
const WALK_STOP_DELAY = 250;

function SkyRayRenderer({
  id,
  position,
  rotation,
  health,
  ...
  useEffect(() => {
    ...
    const handleSkyrayTelegraph = (data: {
      skyrayId?: string;
      hitDelayMs?: number;
      swingLockMs?: number;
      attackRange?: number;
      ...
      if (data.skyrayId !== id) return;
      if (attackTimer.current) clearTimeout(attackTimer.current);
      const visual = parseMeleeTelegraphPayload(data, SKYRAY_MELEE_ATTACK_RANGE, ATTACK_DURATION);
    ...
    const handleSkyrayWhiff = (data: { skyrayId?: string }) => {
      if (data.skyrayId !== id) return;
      setMeleeTelegraph((prev) => (prev ? { ...prev, whiffed: true } : prev));
    };

    socket.on('skyray-attack-telegraph', handleSkyrayTelegraph);
    socket.on('skyray-attack-whiff', handleSkyrayWhiff);
    return () => {
      socket.off('skyray-attack-telegraph', handleSkyrayTelegraph);
      socket.off('skyray-attack-whiff', handleSkyrayWhiff);
    };
  }, [id, socket]);
  ...
  useFrame((_, delta) => {
    ...
    // Treat remaining lerp distance as movement so Swim starts even before
    // enemyTransformsRef is seeded with the first wander batch.
    const effectiveDist =
      dist > 0.01 ? dist : group.position.distanceTo(targetPosition.current);
  ...
  return (
    <group ref={setGroupRef} visible={!isDying || opacity.current > 0}>
      <SkyRayModel
        isWalking={isWalking && !isAttacking && !isDying}
        isAttacking={isAttacking}
        isDying={isDying}
      />
      {isAttacking && !isDying && (
        <EnemyMeleeAttackRangeRing
          radius={meleeTelegraph?.attackRange ?? SKYRAY_MELEE_ATTACK_RANGE}
...
export default React.memo(SkyRayRenderer);
