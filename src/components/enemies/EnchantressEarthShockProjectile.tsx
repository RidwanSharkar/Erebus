...
/** Light-green earth shock bolt — mirrors allied enchantress server projectile sim. */
export interface EnchantressEarthShockProjectileProps {
...
const SPEED = 11;
const trailColor = new Color('#22c55e');
const trailAccent = new Color('#86efac');

export default function EnchantressEarthShockProjectile({
  startPosition,
  targetPosition,
  onComplete,
}: EnchantressEarthShockProjectileProps) {
  ...
  useFrame((_, delta) => {
    ...
    earthLight.current?.setPosition(gp.x, gp.y, gp.z);
    earthLight.current?.setIntensity(16);
    ...
    if (t >= maxLifetimeRef.current) {
      ...
      earthLight.current?.setIntensity(0);
      onComplete();
    }
  });
