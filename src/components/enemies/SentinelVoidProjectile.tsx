...
export interface SentinelVoidProjectileProps {
...
const SPEED = 10;
const trailColor = new Color('#aa44ff');
const trailAccent = new Color('#dd88ff');

export default function SentinelVoidProjectile({
  startPosition,
  targetPosition,
  onComplete,
}: SentinelVoidProjectileProps) {
  ...
  const voidLight = useDynamicLight({ color: '#8844ff', distance: 6.5, priority: 1, intensity: 0 });
  ...
  useFrame((_, delta) => {
    ...
    voidLight.current?.setPosition(gp.x, gp.y, gp.z);
    voidLight.current?.setIntensity(16);
    const pulse = 0.85 + 0.15 * Math.sin(t * 20);
    if (coreRef.current) coreRef.current.scale.setScalar(pulse);
    if (t >= maxLifetimeRef.current) {
      doneRef.current = true;
      voidLight.current?.setIntensity(0);
      onComplete();
    }
  });
