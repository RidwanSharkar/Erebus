...
interface StoneGiantModelProps {
  isWalking: boolean;
  isAttacking: boolean;
  attackVariant: 1 | 2;
  isDying: boolean;
...
type StoneGiantClip = 'Idle' | 'Walk' | 'Melee' | 'Melee2' | 'Death';
type StoneGiantDeferredClip = Exclude<StoneGiantClip, 'Idle'>;

const IDLE_PATH = '/models/giant/AncientGiant_idle.glb';
const MODEL_PATHS = [
  IDLE_PATH,
  '/models/giant/AncientGiant_walk.glb',
  '/models/giant/AncientGiant_attack1.glb',
  '/models/giant/AncientGiant_attack2.glb',
  '/models/giant/AncientGiant_death.glb',
];

const ANIM_PATHS: Record<StoneGiantDeferredClip, string> = {
  Walk: '/models/giant/AncientGiant_walk.glb',
  Melee: '/models/giant/AncientGiant_attack1.glb',
  Melee2: '/models/giant/AncientGiant_attack2.glb',
  Death: '/models/giant/AncientGiant_death.glb',
};

// Bust poisoned multi-clip caches from before pickWowClip (last clip was EmoteStunNoSheathe).
invalidateProcessedClipCache('stone-giant');

export function preloadStoneGiantModels(): void {
...
const BIND_HEIGHT = 7.74;
const TARGET_HEIGHT = 5.5;
const SCALE = TARGET_HEIGHT / BIND_HEIGHT;

export default React.memo(function StoneGiantModel({
  isWalking,
  isAttacking,
  attackVariant,
  isDying,
}: StoneGiantModelProps) {
  const sceneGroupRef = useRef<Group>(null);
  const currentActionRef = useRef<AnimationAction | null>(null);
  const extraActionsRef = useRef<Partial<Record<StoneGiantDeferredClip, AnimationAction>>>({});
  const isMountedRef = useRef(true);
  const [deferredAnimationClips, setDeferredAnimationClips] = useState<
    Partial<Record<StoneGiantDeferredClip, AnimationClip[]>>
  >({});
  ...
  const clonedScene = useMemo(() => {
    return cloneEnemySceneWithSharedMaterials(scene, '/models/giant/AncientGiant_idle.glb', {
  ...
  const idleClips = useMemo(() => {
    ...
    const processed = getCachedProcessedClips('stone-giant-idle', picked, {
  ...
  useEffect(() => {
    ...
    registerClip('Walk', deferredAnimationClips.Walk, 'stone-giant-walk', ['Walk (ID 4 variation 0)', 'Walk'], {
      stripRootMotion: true,
      renameTo: 'Walk',
    });
    registerClip(
    ...
    registerClip(
      ...
      'stone-giant-melee2',
      ['AttackUnarmed (ID 16 variation 1)', 'AttackUnarmed'],
      { stripRootMotion: true, renameTo: 'Melee2' },
    );
    registerClip('Death', deferredAnimationClips.Death, 'stone-giant-death', ['Death (ID 1 variation 0)', 'Death'], {
      renameTo: 'Death',
    });
  }, [deferredAnimationClips, mixer]);

  const getAction = (name: StoneGiantClip): AnimationAction | null =>
    idleActions[name] ?? extraActionsRef.current[name as StoneGiantDeferredClip] ?? null;
