...
interface StonePileDropEffectProps {
  drop: StoneDropLike;
...
const PEBBLE_RADIUS = 0.25;
const SHADOW_RADIUS = 1.55;

const PEBBLE_GEOMETRY = new IcosahedronGeometry(PEBBLE_RADIUS, 0);
const PEBBLE_MATERIAL = new MeshStandardMaterial({
  color: '#8b939c',
  emissive: '#4a5058',
...
export default function StonePileDropEffect({
  drop,
  playerPositionRef,
  onPickup,
  pickupRadius = DEFAULT_PICKUP_RADIUS,
}: StonePileDropEffectProps) {
  const meshRef = useRef<InstancedMesh>(null);
  const pieceCount = Math.max(1, Math.min(VISUAL_PIECE_CAP, Math.floor(drop.pieceCount || drop.amount || 1)));

  const seeds = useMemo<StonePieceAnimSeed[]>(() => {
    const out: StonePieceAnimSeed[] = [];
    for (let i = 0; i < pieceCount; i += 1) {
      const ring = (i / Math.max(pieceCount, 1)) * Math.PI * 2;
      const ringR = 0.55 + (i % 7) * 0.14 + Math.random() * 0.08;
      const scaleXZ = 0.92 + Math.random() * 0.28;
      const scaleY = 0.68 + Math.random() * 0.18;
      out.push({
        startX: Math.cos(ring) * (0.85 + Math.random() * 0.35),
        startY: 0.95 + Math.random() * 0.4,
        startZ: Math.sin(ring) * (0.85 + Math.random() * 0.35),
        targetX: Math.cos(ring) * ringR,
        targetZ: Math.sin(ring) * ringR,
        targetY: PEBBLE_RADIUS * scaleY,
  ...
  return (
    ...
    >
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.02, 0]}>
        ...
        <meshBasicMaterial color="#2a2d32" transparent opacity={0.42} />
      </mesh>
      <instancedMesh
        ref={meshRef}
        args={[PEBBLE_GEOMETRY, PEBBLE_MATERIAL, pieceCount]}
        frustumCulled={false}
      />
    </group>
  );
